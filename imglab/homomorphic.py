...
def imclearborder(imgBW, radius):
    ...
    image, contours, hierarchy= cv2.findContours(imgBWcopy.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def bwareaopen(imgBW, areaPixels):
    # Given a black and white image, first find all of its contours
    imgBWcopy = imgBW.copy()
    _, contours, hierarchy= cv2.findContours(imgBWcopy.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
If = np.fft.fft2(imgLog.copy(), (M,N))
Ioutlow = np.abs(np.fft.ifft2(If.copy() * HlowShift, (M,N)))
Iouthigh = np.abs(np.fft.ifft2(If.copy() * HhighShift, (M,N)))
